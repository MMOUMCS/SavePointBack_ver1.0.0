...
### 백엔드 개선 추가.
**1. IdNot**
UserRepository에서 findByCoupleIdAndIdNot 을 통해 커플 Id 검색 중 내 아이디를 제외하고 상대 아이디만 출력되도록 생성.

**2. 썸네일러 사용.**
프로필 이미지를 업로드 할 시 Thumnails.of()를 사용하여 150x150크기, 품질 70%로 압축하여 저장. S3 스토리지 비용과 네트워크 전송 비용 감소 및 로딩 속도 개선을 위함.
이미지 파일 용량을 약 95% 이상 획기적으로 줄여 서버 비용 절감, 클라이언트의 로딩 속도 극대화.

**3. S3 이미지 업데이트 순서**
프로필 사진을 바꿀 때, oldImageUrl을 미리 변수에 저장해 두고, 새 이미지를 올린 후, 옛 프로필 사진을 지우도록 설계.
예외나 네트워크 발생 시, 기존 이미지와 새 이미지 전부가 유실되는 상태 발생. 이 경우를 막기 위해 사진을 정상적으로 업로드 후 옛날 이미지를 지우도록 예외 안정성을 고려하여 설계.

**4. JPA readOnly = true**
jpa는 db를 꺼내 올 때 마다 스냅샷을 남기고 이후에 더티 체킹을 함. 여기서
readOnly = true"를 주어서 대조나 메모리 낭비를 줄여 서버가 가볍고 빨라지도록 개선.

**5. 회원가입 시 중복검사**
회원가입시에 findByEmail로 존재하는 이메일 일시 예외처리.
